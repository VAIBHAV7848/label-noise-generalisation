\begin{table*}[t]
\centering
\caption{Exploratory Statistical Testing of Accuracy Differences vs Uncorrected Cross-Entropy (CE) across $N=3$ seeds ($\text{df}=2$): Paired $t$-test, Exact $p$-values, Holm-Bonferroni Corrected $p$, and Effect Sizes (Cohen's $d$). Significance is exploratory given small sample size.}
